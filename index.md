...
- [OpenBaoRaftQuorumAtRisk (pre-existing, occurrence #4) — false positive: single-node dev OpenBao where…](incidents/openbaoraftquorumatrisk-pre-existing-occurrence-4-false-positive-single-node-dev-openbao-where-f8c54dff.md) — The alert fires on vault_autopilot_failure_tolerance < 1, but this OpenBao deployment is a single-node dev instance where failure_tolerance=0 is the mathematically correct, inherent value — not a quorum degradation. The alert threshold is inappropriate for this topology.
