...
- [Harbor HelmRelease UpgradeFailed: IAM Access Key quota blocking harbor-registry credentials (not a capacity issue)](incidents/harbor-helmrelease-upgradefailed-iam-access-key-quota-blocking-harbor-registry-credentials-not-a-capacity-issue-c5230eb8.md) — IAM Access Key quota exceeded (AccessKeysPerUser: 2) for the Crossplane provider's IAM user, preventing AccessKey/xplane-harbor from creating the credentials Secret that harbor-registry needs
